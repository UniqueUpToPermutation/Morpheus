...
namespace Morpheus {
	...
	>
	void CreateGLTFPBRStaticMeshPipeline(
		...
		auto pbrStaticMeshVS = LoadShader(device, 
			...
			"internal/GLTFStaticMeshPBR.vsh",
			"GLTF StaticMesh PBR VS",
			"main",
			&newOverrides,
			shaderLoader);
		...
		auto pbrStaticMeshPS = LoadShader(device,
			...
			"internal/GLTFStaticMeshPBR.psh",
			"GLTF StaticMesh PBR PS",
			"main",
			&newOverrides,
			shaderLoader);
		...
		ImtblSamplers.emplace_back(DG::ImmutableSamplerDesc{
			DG::SHADER_TYPE_PIXEL, "g_ColorMap_sampler", SamLinearWrapDesc
		});
		ImtblSamplers.emplace_back(DG::ImmutableSamplerDesc{
			DG::SHADER_TYPE_PIXEL, "g_RoughnessMap_sampler", SamLinearWrapDesc
		});
		ImtblSamplers.emplace_back(DG::ImmutableSamplerDesc{
			DG::SHADER_TYPE_PIXEL, "g_MetallicMap_sampler", SamLinearWrapDesc
		});
		ImtblSamplers.emplace_back(DG::ImmutableSamplerDesc{
			DG::SHADER_TYPE_PIXEL, "g_NormalMap_sampler", SamLinearWrapDesc
		});

		if (bUseAO) {
			ImtblSamplers.emplace_back(DG::ImmutableSamplerDesc{
				DG::SHADER_TYPE_PIXEL, "g_AOMap_sampler", SamLinearWrapDesc
			});
		}
		
		if (bUseEmissive) {
			ImtblSamplers.emplace_back(DG::ImmutableSamplerDesc{
				DG::SHADER_TYPE_PIXEL, "g_EmissiveMap_sampler", SamLinearWrapDesc
			});
		}
